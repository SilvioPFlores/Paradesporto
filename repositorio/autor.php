...
menu($lang['repositUp']);
if(isset($_GET['l'])){
    $arrAutor = buscaAutorLetra($conn,array(':letra' => $_GET['l'].'%'));
    $l = $_GET['l'];
}
else if(isset($_GET['b'])){
    $arrAutor = buscaAutorStr($conn,array(':str' => "%".$_GET['b']."%"));
}
else{
    $arrAutor = buscaAutor($conn);
    $l = '';
}
?>
<div class="container">
    <div class="col-xl-8 col-lg-10 col-md-11 col-sm-12 divCentro">
        ...
        buscaPor($lang, $l, 'autor.php');
        
        if (empty($arrAutor)) {
            echo $lang['nada'];
        }
        foreach($arrAutor as $dadoAutor){
            echo "<a class='btn btnAzul bordAzul espTop4' href='titulo.php?a=$dadoAutor[0]'>$dadoAutor[1]</a> ";
        }
        ?>
    </div>
    <br>
